...
def get_workflow_session_dependency_edges(workflow_elem, session_lookup):
    """
    Returns the actual session dependency edges from WORKFLOWLINK.
    This is used for the session-condition table so conditions are not lost
    when runtime order is de-duplicated by topological sorting.
    """
    task_instance_to_session = get_task_instance_session_map(workflow_elem)
    rows = []

    for link in direct_children(workflow_elem, "WORKFLOWLINK"):
        ...
        if to_task not in task_instance_to_session:
            continue

        from_session = task_instance_to_session.get(
            from_task,
            "Start" if from_task == "Start" else from_task
        )
        to_session = task_instance_to_session[to_task]
...
def get_workflow_session_links_runtime_order(workflow_elem, session_lookup):
    """
    Derives dependency-aware session runtime order from WORKFLOWLINK using
    topological sorting instead of recursive path walking.

    Key behavior:
      - Each session appears once in the returned runtime order.
      - A session is emitted only after all reachable upstream workflow
        dependencies have been emitted.
      - Branch/join workflows are handled correctly, e.g. a join session with
        multiple predecessors is placed after all of its predecessors.
      - The original XML TASKINSTANCE order is used only as a deterministic
        tie-breaker between parallel sessions that are both ready to run.
      - Non-session tasks are retained in the graph for dependency calculation
        but filtered out from the final returned session list.
    """
    task_instance_to_session = get_task_instance_session_map(workflow_elem)
    links = direct_children(workflow_elem, "WORKFLOWLINK")

    # Build deterministic node order: Start first, then TASKINSTANCE XML order,
    # then any link-only endpoints not already seen.
    node_order = []
    ...
    for task_inst in direct_children(workflow_elem, "TASKINSTANCE"):
        add_node(attr(task_inst, "NAME"))

    for link in links:
        add_node(attr(link, "FROMTASK"))
        add_node(attr(link, "TOTASK"))

    order_index = {node: idx for idx, node in enumerate(node_order)}

    # Build full workflow graph including non-session tasks.
    ...
    for link in links:
        ...
        if to_task not in adjacency[from_task]:
            adjacency[from_task].append(to_task)

    # If Start exists, restrict ordering to tasks reachable from Start.
    # Otherwise, process all nodes as a fallback for unusual exports.
    if "Start" in adjacency:
        ...
        nodes_to_sort = reachable
    else:
        nodes_to_sort = nodes

    # Recalculate indegree within the selected reachable subgraph.
    topo_indegree = {node: 0 for node in nodes_to_sort}
    topo_adjacency = defaultdict(list)

    for from_task, next_nodes in adjacency.items():
        if from_task not in nodes_to_sort:
            continue

        for to_task in next_nodes:
            if to_task not in nodes_to_sort:
                continue

            topo_adjacency[from_task].append(to_task)
            topo_indegree[to_task] += 1

    ready = deque(sorted(
        [node for node, degree in topo_indegree.items() if degree == 0],
        key=lambda node: order_index.get(node, len(order_index))
    ))
    ...
    while ready:
        ...
        for next_node in sorted(
            topo_adjacency.get(node, []),
            key=lambda item: order_index.get(item, len(order_index))
        ):
            topo_indegree[next_node] -= 1

            if topo_indegree[next_node] == 0:
                ready.append(next_node)

    # Handle cycles or malformed workflow links by appending remaining nodes
    # in deterministic XML order.
    for node in sorted(
        nodes_to_sort,
        key=lambda item: order_index.get(item, len(order_index))
    ):
    ...
    dependency_edges = get_workflow_session_dependency_edges(
        workflow_elem,
        session_lookup
    )
    incoming_by_session = defaultdict(list)

    for edge in dependency_edges:
        incoming_by_session[edge["to_session"]].append(edge)

    runtime_links = []
    emitted_sessions = set()

    for node in topo_nodes:
        if node not in task_instance_to_session:
            continue

        session_name = task_instance_to_session[node]

        if session_name in emitted_sessions:
            continue

        emitted_sessions.add(session_name)

        incoming_edges = incoming_by_session.get(session_name, [])
        from_sessions = unique_preserve_order([
            edge["from_session"] for edge in incoming_edges
        ])
        conditions = unique_preserve_order([
            edge["condition"] for edge in incoming_edges
        ])

        runtime_links.append({
            "from_session": ", ".join(from_sessions),
            "to_session": session_name,
            "condition": " | ".join(conditions) if conditions else "[condition: none defined]",
            "mapping_name": session_lookup.get(session_name, "")
        })
...
def build_session_condition_table(root, session_lookup):
    ...
    for workflow in workflows:
        ...
        session_links = get_workflow_session_dependency_edges(
            workflow,
            session_lookup
        )
