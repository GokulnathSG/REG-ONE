...
# --------------------------------------------------------------------------
# Per-type eligibility decision logic (mirrors lineage_engine.py exactly)
# --------------------------------------------------------------------------

# Transformation types that are STRUCTURAL eligibility logic by definition:
# their condition/expression is always about record routing/disposition, so
# any non-blank condition is reported without further keyword filtering.
_ALWAYS_INCLUDE_TYPES = {"FILTER", "ROUTER", "UPDATE STRATEGY"}

# Same keyword set as lineage_engine.ELIGIBILITY_KEYWORDS - used for the
# heuristic types (Lookup, Expression) that are NOT inherently eligibility.
_ELIGIBILITY_KEYWORDS_ENGINE = {
...
# Detect a WHERE clause in SQL Override text (Source Qualifier pre-filter).
_WHERE_RE = re.compile(r"\bWHERE\b", re.IGNORECASE)

# Detect control-flow operators that indicate a record-disposition decision
# rather than a simple pass-through derivation.
_CONTROL_LOGIC_RE = re.compile(
    r"\bIIF\s*\(|\bDECODE\s*\(|\bCASE\b|>=|<=|<>|!=|\bIN\s*\(|\bBETWEEN\b|=\s*'",
    re.IGNORECASE,
)

# Extended variant for Lookup: bare `=` (the standard Lookup Condition join
# operator, e.g. LKP_X.PLAN_CD = IN_PLAN_CD) is also treated as control logic
# since every Lookup Condition is by definition a join predicate.
_CONTROL_LOGIC_LOOKUP_RE = re.compile(
    r"\bIIF\s*\(|\bDECODE\s*\(|\bCASE\b|>=|<=|<>|!=|\bIN\s*\(|\bBETWEEN\b|=\s*'|(?<![<>!=])=(?![>=])",
    re.IGNORECASE,
)


def _has_eligibility_keyword(text):
    """True when `text` contains at least one keyword from the engine keyword
    set (same set as lineage_engine.ELIGIBILITY_KEYWORDS)."""
    if not text:
        return False
    t = text.lower()
    return any(kw in t for kw in _ELIGIBILITY_KEYWORDS_ENGINE)


def _has_control_logic(text):
    """True when `text` contains at least one control-flow operator."""
    if not text:
        return False
    return bool(_CONTROL_LOGIC_RE.search(text))


def _has_control_logic_lookup(text):
    """Extended control-logic check for Lookup rows: bare `=` is also
    accepted since every Lookup Condition is a join predicate using `=`."""
    if not text:
        return False
    return bool(_CONTROL_LOGIC_LOOKUP_RE.search(text))


def _looks_like_eligibility(label, text):
    """Heuristic filter for transformation types (Lookup, Expression) that
    are NOT inherently eligibility-related. Mirrors
    lineage_engine._looks_like_eligibility exactly: both a keyword from the
    engine keyword set AND a control-flow operator must be present."""
    if not text:
        return False
    combined = f"{label} {text}"
    return _has_eligibility_keyword(combined) and _has_control_logic(text)


def _should_include_for_anchor(anchor, label, text):
    """Per-type eligibility decision, matching lineage_engine.py's
    _scan_transformation_for_eligibility rules:

    - FILTER / ROUTER / UPDATE STRATEGY: always include when non-blank
      (structural eligibility - their entire purpose is record disposition).
    - SOURCE QUALIFIER: always include when non-blank (the scanner already
      pre-filtered to WHERE-clause SQL / non-blank Source Filter rows).
    - LOOKUP: keyword heuristic + extended control-logic check that also
      accepts bare `=` (the standard Lookup Condition join operator).
    - EXPRESSION: keyword + control-logic heuristic only.
    """
    if not text:
        return False
    if anchor in _ALWAYS_INCLUDE_TYPES or anchor == "SOURCE QUALIFIER":
        return True
    if anchor == "LOOKUP":
        combined = f"{label} {text}"
        return _has_eligibility_keyword(combined) and _has_control_logic_lookup(text)
    # EXPRESSION: require keyword + control logic
    return _looks_like_eligibility(label, text)
...
def _row_candidates_lookup(cols, r):
    ...
    # Combine Condition + SQL Override into ONE candidate (mirrors
    # lineage_engine.py's `"; ".join(parts)` approach) so the eligibility
    # heuristic sees the full context. A Lookup Condition alone uses bare
    # `=` which would fail the control-logic check in isolation even when the
    # transformation is clearly eligibility-related.
    parts = []
    if cond:
...
def _row_candidates_source_qualifier(cols, r):
    ...
    out = []
    # Match lineage_engine.py: only include SQL Override when it contains a
    # WHERE clause (a bare SELECT without a filter is not eligibility logic).
    if sql and _WHERE_RE.search(sql):
        out.append((f"SQL Override: {sql}", assoc))
    # Source Filter is always eligibility-relevant when non-blank.
    if filt:
    ...
    for mapplet_mode in (False, True):
        for anchor in _ANCHORS:
            ...
            trans_col = _find_col(cols, ["Transformation Name", "Instance Name",
                                          ...
                                          "Lookup Transformation Name", "Lookup Instance Name"])
            # Expression sheets have one row per port; read the port name so the
            # output matches lineage_engine.py's "inst_name.port" granularity.
            port_col = (_find_col(cols, ["Port Name", "Output Port Name", "Field Name",
                                          "Port", "Attribute Name"])
                        if anchor == "EXPRESSION" else None)
            # Lookup fallback: when the instance-name column is absent or blank,
            # use the Lookup Table Name as the identifier (mirrors lineage_engine.py's
            # `lkp_table = ... or inst_name` convention).
            lookup_table_col = (_find_col(cols, ["Lookup Table Name", "Lookup table name",
                                                  "LookupTableName"])
                                 if anchor == "LOOKUP" else None)
            scanner = _ROW_SCANNERS[anchor]

            # Forward-fill identifier columns to handle Excel merged cells:
            # when a transformation spans multiple port rows its name is often
            # stored in a merged cell, which pandas reads as NaN after row 1.
            df = df.copy()
            for _ff_col in (trans_col, mapping_col, session_col, mapplet_col, lookup_table_col):
                if _ff_col and _ff_col in df.columns:
                    df[_ff_col] = df[_ff_col].ffill()

            for _, r in df.iterrows():
                try:
                    trans_name = _clean(r.get(trans_col)) if trans_col else ""
                    if anchor == "LOOKUP" and not trans_name and lookup_table_col:
                        # Fall back to the physical Lookup Table Name so the output
                        # column is never blank (matches lineage_engine.py behaviour).
                        trans_name = _clean(r.get(lookup_table_col)) or ""
                    if anchor == "EXPRESSION" and port_col:
                        _port_name = _clean(r.get(port_col))
                        if _port_name:
                            trans_name = (f"{trans_name}.{_port_name}" if trans_name
                                          else _port_name)
                    ...
                    for text, row_table_hint in candidates:
                        if not _should_include_for_anchor(anchor, trans_name, text):
                            continue
...
(function(){
  ...
  // Register column-filter slots in the shared columnFilters map defined by
  // build_lineage.py's HTML. This lets renderHeadWithFilters /
  // openFilterDropdown drive these tabs identically to Tab-1 through Tab-4.
  if(typeof columnFilters !== 'undefined'){
    columnFilters['excel_elig'] = columnFilters['excel_elig'] || {};
    columnFilters['excel_elig_summary'] = columnFilters['excel_elig_summary'] || {};
  }
  ...
  window.renderEligDetail = function(){
    ...
    if(!head || !body) return;
    if(typeof renderHeadWithFilters !== 'undefined'){
      renderHeadWithFilters('excel_elig', head, ELIG_DETAIL_COLS, eligDetailRows,
                             window.renderEligDetail, null);
    } else {
      head.innerHTML = ELIG_DETAIL_COLS.map(c => '<th>' + esc(c) + '</th>').join('');
    }
    const q = (document.getElementById('elig-search').value || '').toLowerCase();
    let rows = eligDetailRows;
    if(typeof rowMatchesColumnFilters !== 'undefined'){
      rows = rows.filter(r => rowMatchesColumnFilters('excel_elig', r));
    }
    rows = rows.filter(r => !q || ELIG_DETAIL_COLS.some(c => String(r[c] ?? '').toLowerCase().includes(q)));
  ...
  window.renderEligSummary = function(){
    ...
    if(!head || !body) return;
    if(typeof renderHeadWithFilters !== 'undefined'){
      renderHeadWithFilters('excel_elig_summary', head, ELIG_SUMMARY_COLS, eligSummaryRows,
                             window.renderEligSummary, null);
    } else {
      head.innerHTML = ELIG_SUMMARY_COLS.map(c => '<th>' + esc(c) + '</th>').join('');
    }
    let rows = eligSummaryRows;
    if(typeof rowMatchesColumnFilters !== 'undefined'){
      rows = rows.filter(r => rowMatchesColumnFilters('excel_elig_summary', r));
    }
    body.innerHTML = rows.map(r =>
