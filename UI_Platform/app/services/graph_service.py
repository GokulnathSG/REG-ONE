...
def reportability_graph_html_by_mapping(graph_data: dict, grouping: dict) -> dict:
    """Raw markup of each Mapping panel's reportability graph, for embedding
    into the standalone downloaded Reportability HTML export."""
    paths = {}
    with tempfile.TemporaryDirectory() as tmp:
        for mapping_name in grouping["order"]:
            node_ids = grouping["panels"][mapping_name]["node_ids"]
            g = build_flow_subgraph_from_dict(graph_data["nodes"], graph_data["edges"], node_ids)
            safe_mapping = "".join(c if c.isalnum() else "_" for c in mapping_name) or "mapping"
            out = os.path.join(tmp, f"{safe_mapping}.html")
            export_pyvis_html(g, out, hierarchical=True, directional=True)
            with open(out, "r", encoding="utf-8") as handle:
                paths[mapping_name] = handle.read()
    return paths