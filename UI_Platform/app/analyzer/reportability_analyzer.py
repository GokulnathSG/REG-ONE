...
from app.services import reportability_logic_service as rls

MAX_HOPS = fla.MAX_HOPS


def _logic_override(mapplet_name: str, transformation_name: str, field_name: str,
                    transformation_type: str) -> str:
    if mapplet_name:
        logic = rls.get_mapplet_logic(mapplet_name, transformation_name, field_name, transformation_type)
        if logic:
            return logic
    logic = rls.get_transformation_logic(transformation_name, field_name, transformation_type)
    return logic or ""


def _expression_reference(mapplet_name: str, transformation_name: str, field_name: str,
                          transformation_type: str) -> str:
    if mapplet_name:
        ref = rls.get_mapplet_expression_reference(mapplet_name, transformation_name, field_name, transformation_type)
        if ref:
            return ref
    ref = rls.get_transformation_expression_reference(transformation_name, field_name, transformation_type)
    return ref or ""
    ...
    def process(ctx: fla._Ctx, owning_mapping: str, owning_session: Optional[str],
        ...
        try:
            ...
            if inst.type == "MAPPLET":
                ...
                logic_text = _logic_override(inst.ref_name, instance_name, field_name, "Mapplet")
                nodes[node_id] = {
                    "id": node_id, "kind": "MAPPLET",
                    "label": f"{instance_name}\n[{inst.ref_name}]",
                    "mapping": owning_mapping, "session": owning_session or "",
                    "instance": instance_name, "table": "", "field": field_name,
                    "logic": logic_text or f"Enters Mapplet '{inst.ref_name}' via its output port '{field_name}'.",
                    "passthrough": True,
                }
            ...
            uploaded_logic = _logic_override(ctx.name if ctx.kind == "mapplet" else "", instance_name,
                                             field_name, ttype)
            if uploaded_logic:
                logic_text = uploaded_logic
            workbook_ref = _expression_reference(ctx.name if ctx.kind == "mapplet" else "",
                                                 instance_name, field_name, ttype)
            if workbook_ref and workbook_ref not in ref_ports:
                ref_ports = list(ref_ports) + [workbook_ref]
