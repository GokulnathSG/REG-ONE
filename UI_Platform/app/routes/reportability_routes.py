...
def reportability_page():
    ...
    if not mapping_name and not table_name:
        ...
        return render_template("reportability.html", repo=repo, logic_workbook_name=rls.get_workbook_name())
    ...
    ctx = {"repo": repo, "logic_workbook_name": rls.get_workbook_name()}
...
def reportability_submit():
    ...
    workbook = request.files.get("logic_workbook")

    if workbook and (workbook.filename or "").strip():
        rls.load_workbook_from_upload(workbook)
    else:
        rls.clear_workbook()
...
@bp.route("/reportability/download")
def download_reportability_html():
    repo = require_repo()
    mapping_name = (request.args.get("mapping") or "").strip()
    table_name = (request.args.get("table") or "").strip()
    instance_name = (request.args.get("instance") or "").strip()

    ctx = _resolve_and_list(repo, mapping_name, table_name, instance_name)
    if not ctx.get("ready"):
        return render_template("reportability.html", repo=repo, logic_workbook_name=rls.get_workbook_name(), **ctx), 400

    path = document_service.reportability_html_path(
        repo,
        mapping_name,
        ctx["table_name"],
        ctx["instance_name"],
    ...
    safe_name = "".join(c if c.isalnum() or c in "._-" else "_" for c in f"{mapping_name}_{ctx['instance_name']}")
    return send_file(path, as_attachment=True, download_name=f"Reportability_{safe_name or 'Export'}.html")
